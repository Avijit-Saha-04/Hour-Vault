...
createRoomBtn.addEventListener('click', () => {
    ...
    socket.emit('createRoom', { deletionTime });
});
...
messageForm.addEventListener('submit', (e) => {
    ...
    if (message && currentRoomCode) {
        // 1. Encrypt the message with the room code
        const encryptedMessage = encryptMessage(message, currentRoomCode);

        // 2. Send the ENCRYPTED message to the server
        socket.emit('sendMessage', { roomCode: currentRoomCode, encryptedMessage });

        // 3. Display your own message immediately (unencrypted)
        displayMessage(message, 'You', 'mine');
